...
{
    public partial class ucDavaHizliAramaFoy : AdimAdimDavaKaydi.Util.BaseClasses.AvpXUserControl// UserControl
    {
        public ucDavaHizliAramaFoy()
        {
            InitializeComponent();
        ...
        public List<AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA> MyDataSource
        {
            get
            {
                if (gridDavaBIL_Foy.DataSource is List<AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA>)
                    return (List<AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA>)gridDavaBIL_Foy.DataSource;
            ...
            set
            {
                ...
                gridDavaBIL_Foy.barCheckHucreBirlestir.Checked = true;
            }
        }
            ...
            if (e.Item.Tag != null)
            {
                ...
                else if (e.Item.Name == bButonBelgeEkle.Name)
                {
                    if (e.Item.Tag is AvukatProLib.Arama.AV001_TD_BIL_FOY)
                    {
                        ...
                        AV001_TD_BIL_FOY takip = DataRepository.AV001_TD_BIL_FOYProvider.GetByID(genel.ID);
                        string tablob = "List<AvukatProLib.Arama.AV001_TD_BIL_FOY>";
                        bool islemYap = true;
                        if (islemYap)
                        {
                            if (takip.ID != null)
                ...
                else if (e.Item.Name == "bButtonKlasorAc")
                {
                    if (e.Item.Tag is AvukatProLib.Arama.AV001_TD_BIL_FOY)
                    {
                        ...
                        TList<AV001_TDIE_BIL_PROJE_DAVA_FOY> icra =
                            DataRepository.AV001_TDIE_BIL_PROJE_DAVA_FOYProvider.GetByDAVA_FOY_ID(takip.ID);
                        if (icra.Count > 0)
                        {
        ...
        private List<AvukatProLib.Arama.AV001_TD_BIL_FOY> seciliDavaKayitlari;

        public List<AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA> GetSelectedFoy(List<AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA> foy)
        {
            return foy.Where(vi => vi.IsSelected).ToList();
        }

        public List<AvukatProLib.Arama.AV001_TD_BIL_FOY> GetSelectedFoy(List<AvukatProLib.Arama.AV001_TD_BIL_FOY> foy)
        {
            seciliDavaKayitlari = new List<AvukatProLib.Arama.AV001_TD_BIL_FOY>();
            foreach (AvukatProLib.Arama.AV001_TD_BIL_FOY f in foy)
            {
                if (f.IsSelected)
        ...
        private void gridDavaBIL_Foy_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA takipV = gridView1.GetFocusedRow() as AvukatProLib.Arama.per_DAVA_HIZLI_ARAMA;
                AvukatProLib2.Entities.AV001_TD_BIL_FOY takip = DataRepository.AV001_TD_BIL_FOYProvider.GetByID(takipV.ID);
                frmDavaTakip frmdavaTakip = new frmDavaTakip();
        ...
        public int DosyaSayisi
        {
            ...
            set
            {
                ...
                {
                    _DosyaSayisi = value;
                    groupControl1.Text = "Dava Dosya Bilgileri (Dosya Sayısı = " + _DosyaSayisi.ToString() + ")";
                }
            }
        }
    }
