...
namespace AvukatproUpdater.Common.FileOperations
{
    public class FileUtilityTest
    {
        ...
        public FileUtilityTest(UpdaterServiceBase serviceBase)
        {
            this.serviceBase = serviceBase;
        }

        public FileUtilityTest(UpdateFormBase formBase)
        {
        ...
        public void CopyDirectory(string sourcePath, string destPath, DirectoryCopyType copyType)
        {
            ...
            if (CopyDir(sourcePath, destPath, false))
            {
                ...
                switch (copyType)
                {
                    case DirectoryCopyType.ApplicationBackup: formBase.OperationType = OperationType.ApplicationBackupCompleted; break;
                    case DirectoryCopyType.ApplicationSetup: formBase.OperationType = OperationType.ApplicationSetupCompleted; break;
                }
            }
            else
            {
        ...
        public void CopyDirectoryAsync(string sourcePath, string destPath, ProgressControl progressControl)
        {
            ...
            progressControl.Visible = progressControl.IconVisible = progressControl.DownloadMode = true;
        ...
        public void CopyFileAsync(string sourcePath, string destPath, ProgressControl progressControl)
        {
            ...
            progressControl.ProcessCompleted = false;
            progressControl.Visible = progressControl.DownloadMode = progressControl.IconVisible = true;
        ...
        private void CopyFile(string sourcePath, string destPath, bool isAsync)
        {
            ...
            using (FileStream inFile = new FileStream(sourcePath, FileMode.Open))
            {
                using (FileStream outFile = new FileStream(destPath, FileMode.Create))
                {
                    inFile.Position = 0;
